...
(function() {
  var fieldIds, persistToScreendoor, persistedFromStore, projectId, responseVals, showError, showSuccess, storePrefix, translatedVals, validateNps;
  ...
  showError = function(message) {
    return $('.form_error').text(message).show();
  };
  ...
  translatedVals = function() {
    var k, newVals, v, vals;
    vals = responseVals();
    newVals = {};
    for (k in fieldIds) {
      v = fieldIds[k];
  ...
  persistToScreendoor = function() {
    ...
    return $.ajax({
      ...
      success: function(data) {
        ...
        return store.set(storePrefix + 'id', data.response_id);
      },
      error: function() {
        store.unset(storePrefix + 'responses');
        store.unset(storePrefix + 'id');
        return showError('Error saving your response.');
      }
    });
  };
